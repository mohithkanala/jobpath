# Contexto Compartilhado -- jobpath (Português BR)
...
<!-- ============================================================
     ...
     that improve with each jobpath release.
     ============================================================ -->
...
1b. **Na primeira avaliação de cada sessão:** Executar `node scripts/pipeline/cv-sync-check.mjs` via Bash. Se houver avisos, informar o candidato antes de continuar
...
9. **Entradas no tracker como TSV** — NUNCA editar `applications.md` diretamente para novos registros. Escrever TSV em `batch/tracker-additions/`, `scripts/tracker/merge-tracker.mjs` cuida do merge
...
| Bash | `node scripts/cv/generate-pdf.mjs`, `node scripts/tracker/merge-tracker.mjs` |
