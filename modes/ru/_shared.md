# Общий контекст — jobpath (Русский)
...
<!-- ============================================================
     ...
     (scripts/system/update-system.mjs). Обновления применяются вручную.
     
     Перед использованием jobpath убедитесь:
...
1b. **Первая оценка в сессии:** Запустить `node scripts/pipeline/cv-sync-check.mjs`. Если предупреждения — уведомить пользователя.
...
`scripts/cv/generate-pdf.mjs` автоматически нормализует em-dash, умные кавычки и zero-width символы в ASCII для максимальной ATS-совместимости. Но лучше не генерировать их изначально.
