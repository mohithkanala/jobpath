...
```bash
node scripts/pipeline/followup-cadence.mjs
```
...
> "No active applications to follow up on. Apply to some roles first with `/jobpath` and come back when they're aging."
...
- Suggest the user run `/jobpath contacto {company}` to find the right person first
...
> - Trying a different contact via `/jobpath contacto`
...
**To:** {email or "No contact found — run `/jobpath contacto` first"}
...
These defaults can be overridden via `node scripts/pipeline/followup-cadence.mjs --applied-days N`.
