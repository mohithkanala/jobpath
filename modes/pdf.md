...
15. Ejecuta: `node scripts/cv/generate-pdf.mjs /tmp/cv-{candidate}-{company}.html jobs/cv-{candidate}-{company}-{YYYY-MM-DD}.pdf --format={letter|a4}`
...
b. **IMMEDIATELY** download the PDF using Bash:
   ```bash
   curl -sL -o "jobs/cv-{candidate}-{company}-canva-{YYYY-MM-DD}.pdf" "{download_url}"
   ```
   The export URL is a pre-signed S3 link that expires in ~2 hours. Download it right away.
c. Verify the download:
   ```bash
   file jobs/cv-{candidate}-{company}-canva-{YYYY-MM-DD}.pdf
   ```
