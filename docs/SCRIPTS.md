...
|---------|--------|---------|
| `npm run doctor` | `scripts/system/doctor.mjs` | Validate setup prerequisites |
...
| `npm run sync-check` | `scripts/pipeline/cv-sync-check.mjs` | Validate CV/profile consistency |
| `npm run update:check` | `scripts/system/update-system.mjs check` | Check for upstream updates |
| `npm run update` | `scripts/system/update-system.mjs apply` | Apply upstream update |
| `npm run rollback` | `scripts/system/update-system.mjs rollback` | Rollback last update |
| `npm run liveness` | `scripts/pipeline/check-liveness.mjs` | Test if job URLs are still active |
| `npm run scan` | `scripts/pipeline/scan.mjs` | Zero-token portal scanner |

---

## doctor

Validates that all prerequisites are in place: Node.js >= 18, dependencies installed, Playwright chromium, required files (`cv.md`, `config/profile.yml`, `portals.yml`), fonts directory, and auto-creates `data/`, `jobs/`, `reports/` if missing.
...
Validates that the jobpath setup is internally consistent: `cv.md` exists and is not too short, `config/profile.yml` exists with required fields, no hardcoded metrics in `modes/_shared.md` or `batch/batch-prompt.md`, and `article-digest.md` freshness (warns if older than 30 days).
...
Checks whether a newer version of jobpath is available upstream. Outputs JSON to stdout:
