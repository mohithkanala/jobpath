# jobpath — AI Job Search Pipeline (Gemini CLI)
...
- `{"status": "update-available", ...}` → tell the user an update is available and ask if they want to apply it (`node scripts/system/update-system.mjs apply`)
...
|---------|------------------------|-------------|
| `/jobpath` | `/jobpath` | Show menu or evaluate JD |
| `/jobpath-pipeline` | `/jobpath pipeline` | Process pending URLs from inbox |
| `/jobpath-evaluate` | `/jobpath oferta` | Evaluate job offer (A-G scoring) |
| `/jobpath-compare` | `/jobpath ofertas` | Compare and rank multiple offers |
| `/jobpath-contact` | `/jobpath contacto` | LinkedIn outreach |
| `/jobpath-deep` | `/jobpath deep` | Deep company research |
| `/jobpath-pdf` | `/jobpath pdf` | Generate ATS-optimized CV |
| `/jobpath-training` | `/jobpath training` | Evaluate course/cert |
| `/jobpath-project` | `/jobpath project` | Evaluate portfolio project |
| `/jobpath-tracker` | `/jobpath tracker` | Application status overview |
| `/jobpath-apply` | `/jobpath apply` | Live application assistant |
| `/jobpath-scan` | `/jobpath scan` | Scan portals for new offers |
| `/jobpath-batch` | `/jobpath batch` | Batch processing |
| `/jobpath-patterns` | `/jobpath patterns` | Analyze rejection patterns |
| `/jobpath-followup` | `/jobpath followup` | Follow-up cadence tracker |
...
| `scripts/system/gemini-eval.mjs` | Standalone Gemini API evaluator (no CLI required) |
...
1. **NEVER edit applications.md to ADD new entries** — Write TSV in `batch/tracker-additions/` and `node scripts/tracker/merge-tracker.mjs` handles the merge.
2. Run `node scripts/tracker/verify-pipeline.mjs` to check health.
