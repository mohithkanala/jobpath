...
System-layer template files used by jobpath scripts and modes. These files are auto-updated when you run `npm run update` -- put user customizations in the user-layer files instead (see DATA_CONTRACT.md).

## Files

| File | Used By | Purpose |
|------|---------|---------|
| `cv-template.html` | `scripts/cv/generate-pdf.mjs` | HTML/CSS template for ATS-optimized CV PDFs |
| `cv-template.tex` | `scripts/cv/generate-latex.mjs` | LaTeX/Overleaf template for ATS-optimized CV PDFs |
| `portals.example.yml` | Onboarding | Example portal scanner configuration (copy to `portals.yml` to activate) |
| `states.yml` | `scripts/tracker/verify-pipeline.mjs`, `scripts/tracker/normalize-statuses.mjs`, `scripts/tracker/merge-tracker.mjs` | Canonical application states and their aliases |
...
node scripts/cv/generate-latex.mjs jobs/cv-name-company-date.tex

# Or specify a custom output path
node scripts/cv/generate-latex.mjs jobs/cv-name-company-date.tex jobs/custom-name.pdf
```
